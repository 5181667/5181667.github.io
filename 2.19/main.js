        ...
        if (!chessboard.isEnded()) {
            ...
            if (self.is('.chess')) {
                ...
                if (chessboard.isMaxWin()) {
                    chessboard.end();
                    clearInterval(tick);
                    return alert('你花了' + t + 's打败了AICHI 0.5版');
                }
                //没赢，但平手了
                if (chessboard.isEnded()) {
                    clearInterval(tick);
                    return alert('你花了' + t + 's和AICHI 平局');
                } else {
                    setTimeout(function() {
                        //未分胜负，AI下棋
                        console.time('min');
                        ...
                        if (chessboard.isMinWin()) {
                            chessboard.end();
                            clearInterval(tick);
                            return alert('stupid，你被AICHI0.5 打败了O.o！');
                        }
                        //没赢，但平手了，否则未分胜负，等待人继续下棋
                        if (chessboard.isEnded()) {
                            clearInterval(tick);
                            return alert('你花了' + t + 's打败了AICHI，你已经拥有hc50%的实力');
                        }
                    }, 200);
                }
            }
